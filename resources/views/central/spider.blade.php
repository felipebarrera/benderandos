...
.spider-kpis{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:12px}
.spider-kpi{background:var(--s2);border-radius:var(--r);padding:8px 10px;text-align:center}
.spider-kpi-v{font-family:var(--mono);font-size:20px;font-weight:700}
.spider-kpi-l{font-size:9px;color:var(--t2);margin-top:2px;font-family:var(--mono);text-transform:uppercase}
.kp .spider-kpi-v{color:var(--ok)}.kf .spider-kpi-v{color:var(--err)}.kw .spider-kpi-v{color:var(--warn)}.kb .spider-kpi-v{color:var(--info)}
.spider-layout{display:flex;gap:20px;height: calc(100vh - 120px);overflow:hidden}
...
.spider-fr{display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap}
.spider-fb{font-size:10px;font-family:var(--mono);padding:3px 9px;border-radius:4px;
  border:1px solid var(--b2);background:none;color:var(--t2);cursor:pointer}
.spider-fb.on{border-color:var(--ac);color:var(--ac);background:rgba(224,64,251,.07)}
...
<aside class="spider-sidebar">
  ...
  <div class="f"><label>SA Password</label><input type="password" id="sa-pass" value="password" autocomplete="new-password"></div>
  <div class="f"><label>Tenant Email</label><input id="t-email" value="admin@benderand.cl" autocomplete="off"></div>
  <div class="f"><label>Tenant Password</label><input type="password" id="t-pass" value="admin1234" autocomplete="new-password"></div>
  ...
  <div class="spider-kpis">
    <div class="spider-kpi kp"><div class="spider-kpi-v" id="kp">0</div><div class="spider-kpi-l">PASS</div></div>
    <div class="spider-kpi kf"><div class="spider-kpi-v" id="kf">0</div><div class="spider-kpi-l">FAIL</div></div>
    <div class="spider-kpi kw"><div class="spider-kpi-v" id="kw">0</div><div class="spider-kpi-l">WARN</div></div>
    <div class="spider-kpi kb"><div class="spider-kpi-v" id="kb">0</div><div class="spider-kpi-l">BUGS</div></div>
  </div>
</aside>
<div class="spider-main">
  <div class="tabs">
    ...
    <div class="tab" id="tab-tests" onclick="st('tests')">Tests JSON <span id="json-ct" class="badge" style="display:none; margin-left:6px; background:var(--info)"></span></div>
  ...
  <div class="pane" id="pane-tests">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      ...
      <button class="btn" style="background:var(--s2); border:1px solid var(--b2); color:var(--tx)" onclick="syncTests()">⟳ Re-sync</button>
    </div>
  ...
  <div class="pane" id="pane-md">
    ...
    <div id="mds" style="display:none">
      <div class="brow">
        ...
        <button class="btn bdl" style="background:var(--s2); border:1px solid var(--b2); color:var(--tx)" onclick="dlMd()">Descargar .md</button>
      </div>
...
async function syncTests(){
  ...
  try{
    // Necesita token SA para auth - Intentar obtenerlo desde sesión Laravel primero
    if(!S.saT){
       try {
        ...
        if(d_token.token) S.saT = d_token.token;
      } catch(e) {
        log('Error obteniendo token de sesión: '+e.message, 'err');
      }
    }

    if(!S.saT){
      // Fallback a login silencioso si la sesión no funciona
    ...
    if(!r.ok){
      throw new Error('HTTP '+r.status+' — SpiderController no instalado')
    }
    ...
    log('Sync OK: '+d.after_total+' tests')
  }catch(e){
